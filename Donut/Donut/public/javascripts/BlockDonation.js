...
contractAddress = '0x03b9087e15a6d3ab0e657a67741a2230280b32c0'; 
...
async function give_gasprice2() {
  if(confirm("프로젝트를 등록하시겠습니까?")== true)
  ...
  .on('receipt', function(receipt){$("#state").text('[채굴중] 블록체인 사용을 위한 수수료 지급중');})
  .on('confirmation', function(confirmationNumber, receipt){
    if(confirmationNumber == 1){  
      $("#state").text('[채굴완료] 블록체인에 사용을 위한 수수료 지급 완료');
      register();
    }
  })
...
async function register() {
  $("#state").text('[요청] 프로젝트 정보를 블록체인에 보내는 중');
  ...
  .on('receipt', function(receipt){console.log(receipt)
    $("#state").text('[채굴중] 프로젝트 정보 블록체인에 등록중');
  })
  .on('confirmation', function(confirmationNumber, receipt){
    console.log(confirmationNumber);
    if(confirmationNumber == 1){
      console.log("confirmed")
      $("#state").text('[채굴완료] 프로젝트 정보 블록체인에 등록완료');
    }
  })
  .on('error', function(err){
    console.log(err);
...
function init_confirm(){
  ...
  methods.campaigns_address(address).call().then(l => {
    ...
    if(l[4]==0){l[8]=-1;}
      methods.campaigns(l[8]).call().then(l2 => {
        document.getElementById('campaign_name').innerText =  l2[0] ;
        document.getElementById('project_name').innerText =  l2[1] ;
        document.getElementById('date').innerText =  l2[2] ;
        document.getElementById('realaccount').innerText =  l2[3] ;
        document.getElementById('campaign_address').innerText =  l2[4] ;
        document.getElementById('numFunders').innerText =  l2[5] ;
        document.getElementById('myBalance').innerText =  l2[6] ;
        document.getElementById('fund_goal').innerText =  l2[7] ;
          methods.balanceOf(l2[4]).call().then(l3 =>{
            document.getElementById('fundedmoney').innerText = l3;
      });
      // var campaign_address = document.getElementById('campaign_address'); 
      // console.log(campaign_address)
 
     
      let fg = $("#fund_goal").text();
      let mb = $("#myBalance").text();
      let pp = ((mb/fg) * 100).toFixed(2);
      if(fg == 0){
        pp = 0;
      }
      $("#progressbar").attr("aria-valuenow",pp).text(pp+"%").css("width",pp+"%");
  });
});
}
async function use_money() {
  var campaignID = $("#donationID").text();
  var _amount = document.getElementById('_amount').value;
  var _usage = document.getElementById('_usage').value;
  console.log(campaignID)
  console.log(_amount,_usage)
  let address = $("#address").val();
  let privatekey = $("#privatekey").val();
  if(confirm('후원금을 사용하시겠습니까? 로그인한 계정의 주소인' +address+'에서 '+_amount+"만큼의 도넛이 사라집니다. \n 입금 예정액 : "+(_amount*1000))== true)
  $("#state").text('[요청] 사용정보 블록체인에 보내는 중');
  ...
  .on('receipt', function(receipt){console.log(receipt);
    $("#state").text('[채굴중] Donut 사용 내역 기록중');
  })
  .on('confirmation', function(confirmationNumber, receipt){
    console.log(confirmationNumber);
    if(confirmationNumber == 1){
      $("#state").text('[채굴완료] Donut 사용 완료');
      init_confirm();
    }
  })
  .on('error', function(err){
...
async function donate1() {
  ...
  if(confirm('후원하시겠습니까? 로그인한 계정의 주소인' +address+'에서 '+_amount+"만큼의 도넛이 보내집니다.")== true)
  $("#state").text('[요청] 기부정보 블록체인에 보내는 중');
  await web3.eth.sendTransaction({from:address ,to:contractAddress, value:0, gas:5900000, gasPrice:8000000000, data:enabi})
  .on('transactionHash', function(hash){
    alert('https://rinkeby.etherscan.io/tx/' + hash);
    // document.getElementById('etherscan').innerText = "https://rinkeby.etherscan.io/tx/"+hash ;
  ...
  .on('receipt', function(receipt){console.log(receipt);
    $("#state").text('[채굴중] Donut 후원단체에게 보내는 중');
  })
  .on('confirmation', function(confirmationNumber, receipt){
    console.log(confirmationNumber);
    if(confirmationNumber == 1){
      $("#state").text('[채굴완료] Donut 기부 완료');
      checking_campaigns();
      }
    })
    .on('error', function(err){
  ...
  async function donate2() {
   ...
   console.log(privatekey);
    ...
    await login_privatekey("blockdonation",privatekey);
    // var address = "0x235C276eeDa97d1Aa0CDbc89BDb423CCD5822e57"
    var _amount = document.getElementById('amount2').value;
    if(confirm('후원하시겠습니까? 로그인한 계정의 주소인' +address+'에서 '+_amount+"만큼의 도넛이 보내집니다.")== true)
    $("#state2").text('[요청] 기부정보 블록체인에 보내는 중');
    ...
    .on('receipt', function(receipt){console.log(receipt)
      $("#state2").text('[채굴중] Donut 후원단체에게 보내는 중');
    })
    .on('confirmation', function(confirmationNumber, receipt){
      console.log(confirmationNumber);
      if(confirmationNumber == 1){
        $("#state2").text('[채굴완료] Donut 기부 완료');
        checking_campaigns();
        }
      })
      .on('error', function(err){
    ...
    async function donate3() {
     ...
     console.log(privatekey);
      ...
      await login_privatekey("blockdonation",privatekey);
      // var address = "0x235C276eeDa97d1Aa0CDbc89BDb423CCD5822e57"
      var _amount = document.getElementById('amount3').value;
      if(confirm('후원하시겠습니까? 로그인한 계정의 주소인' +address+'에서 '+_amount+"만큼의 도넛이 보내집니다.")== true)
      $("#state3").text('[요청] 기부정보 블록체인에 보내는 중');      
      ...
      .on('receipt', function(receipt){console.log(receipt);
        $("#state3").text('[채굴중] Donut 후원단체에게 보내는 중');
      })
      .on('confirmation', function(confirmationNumber, receipt){
        console.log(confirmationNumber);
        if(confirmationNumber == 1){
          $("#state3").text('[채굴완료] Donut 기부 완료');
            checking_campaigns();
          }
        })
        .on('error', function(err){
      ...
      async function donate4() {
       ...
       console.log(privatekey);
        ...
        await login_privatekey("blockdonation",privatekey);
        // var address = "0x235C276eeDa97d1Aa0CDbc89BDb423CCD5822e57"
        var _amount = document.getElementById('amount4').value;
        if(confirm('후원하시겠습니까? 로그인한 계정의 주소인' +address+'에서 '+_amount+"만큼의 도넛이 보내집니다.")== true)
        $("#state4").text('[요청] 기부정보 블록체인에 보내는 중');        
        ...
        .on('receipt', function(receipt){console.log(receipt);
          $("#state4").text('[채굴중] Donut 후원단체에게 보내는 중');
        })        
        .on('confirmation', function(confirmationNumber, receipt){
          console.log(confirmationNumber);
          if(confirmationNumber == 1){
            $("#state4").text('[채굴완료] Donut 기부 완료');
            checking_campaigns();
...
async function donate5() {
 ...
 console.log(donationID);
  ...
  if(confirm('후원하시겠습니까? 로그인한 계정의 주소인' +address+'에서 '+_amount+"만큼의 도넛이 보내집니다.")== true)
  $("#state5").text('[요청] 기부정보 블록체인에 보내는 중');  
  ...
  .on('receipt', function(receipt){console.log(receipt);
    $("#state5").text('[채굴중] Donut 후원단체에게 보내는 중');
  })      
  .on('confirmation', function(confirmationNumber, receipt){
    console.log(confirmationNumber);
    if(confirmationNumber == 1){
      $("#state5").text('[채굴완료] Donut 기부 완료');
      checking_myList();
      }
    })
    .on('error', function(err){
            ...
            methods.campaigns(2).call().then(l => {
              ...
              document.getElementById('donationID3').innerText =  l[8] ;
                    methods.campaigns(3).call().then(l => {
                      ...
                      for(let i=1;i<5;i++){
                        let fg = $("#fund_goal"+i).text();
                        let mb = $("#myBalance"+i).text();
                        let pp = ((mb/fg) * 100).toFixed(2);
                        if(fg == 0){
                          pp = 0;
                        }
                        $("#progressbar"+i).attr("aria-valuenow",pp).text(pp+"%").css("width",pp+"%");
                      }
            });
        });
      });
...
function checking_myList(){
  ...
  methods.campaigns(donationID).call().then(l => {
    console.log(l)
      ...
      let fg = $("#fund_goal5").text();
      let mb = $("#myBalance5").text();
      let pp = ((mb/fg) * 100).toFixed(2);
      if(fg == 0){
        pp = 0;
      }
      $("#progressbar5").attr("aria-valuenow",pp).text(pp+"%").css("width",pp+"%");
      // if(pp>100){
      //   console.log("over")
        $("#over").text(pp+"% complete!");
      // }
  });
}
//////////////////////////////////////////////////////MyPage///////////////////////////
