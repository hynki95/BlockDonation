...
contract Donut is ERC20,Ownable {
   ...
   constructor () public {
    ...
    event contribute_log(address contributer, uint campaignID, uint _amount);
    function contribute(uint campaignID, uint _amount) public{
        // campaign storage c2 = campaigns[campaignID].funders[Funer_index];
        ...
        // campaigns[0].funders[campaigns.numFunders] = funder({addr: msg.sender, amount: _amount});
    ...
    event use_money_log(address user,uint send_amount, string usage);
    ...
    function use_money( uint campaignID, uint _amount, string memory _usage) public{
        ...
        //  Trx_usage[Trx_index].user = msg.sender;
